...
const PrimaryTabModal = memo(
  ({ createdAt, pickupTime, user, orderId, items, onConfirm, setKey }) => {
    return (
      <>
        <Modal.Header>
        ...
        <Modal.Body className="p-0 overflow-visible">
          {items.map((i, index) => (
            <Item {...i} index={index} length={items.length} />
          ))}
        </Modal.Body>
...
const OrderModal = memo((props) => {
  const { user, orderId = "FF4FSD", createdAt, onConfirm, setKey } = props;
      ...
      >
        <div className="d-flex px-2 justify-content-between align-items-center">
          <div className="text-start">
            ...
            <div className="fw-normal">{orderId}</div>
          </div>
